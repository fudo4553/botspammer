...
API_HASH = "a4d881ddb8f0a191539d736b90a72c67"
...
async def MessagesManager(e):
	...
	if e.chat_id == ADMIN:
		if e.text == "/start":
			if SpamEnabled:
				await e.respond("**🤖 Spammer Bot v1.3 🤖**\n\n__ℹ️ Stato Spam »__ **Attivo ✅**\n\n__⚙ **", buttons=[[Button.inline("❌ Stoppa", "stop"), Button.inline("Spammer 📞", "voip")], [Button.inline("⏱ Tempo", "timer"), Button.inline("Messaggio 💬", "messaggio")], [Button.inline("👥 Entra nei Gruppi 👥", "join")]])
			else:
				await e.respond("**🤖 Spammer Bot v1.3 🤖**\n\n__ℹ️ Stato Spam »__ **Non Attivo ❌**\n\n__⚙ **", buttons=[[Button.inline("✅ Avvia", "start"), Button.inline("Spammer 📞", "voip")], [Button.inline("⏱ Tempo", "timer"), Button.inline("Messaggio 💬", "messaggio")], [Button.inline("👥 Entra nei Gruppi 👥", "join")]])
		elif Getter != None:
			if Getter == 0:
				Getter = None
	...
	if e.sender_id == ADMIN:
		if e.data == b"back":
			...
			if SpamEnabled:
				...
				await e.edit("**🤖 Spammer Bot v1.3 🤖**\n\n__ℹ️ Stato Spam »__ **Non Attivo ❌**\n\n__⚙ **", buttons=[[Button.inline("✅ Avvia", "stop"), Button.inline("Spammer 📞", "voip")], [Button.inline("⏱ Tempo", "timer"), Button.inline("Messaggio 💬", "messaggio")], [Button.inline("👥 Entra nei Gruppi 👥", "join")]])
		elif e.data == b"voip":
